...
## Binding rows and adding region 2012 entry
...
### add 2012 entry for region
region2012 <- county %>%
  filter(year == 2012) %>%
  summarise(across(HU_TOT:VAC_HU, ~ sum(.x, na.rm = T)))
...
county <- bind_rows(county, region_row)
...
county <- county %>%
  ...
  mutate(infl2022 = 292.655/inflationfac) %>% #292.655 is the 2022 inflation factor 
  mutate(medincome_adjust = infl2022 *MEDINC) %>%
  mutate(medhv_adjust = infl2022 * MED_HV)
```
...
ggplotly(total_units_plot, tooltip = "text") %>%
  layout(
    ...
    title = list(
      text = "Total housing units by county, 2012-2022",
...
```
### housing units and households change
```{r}
# Step 1: Prepare data
hu_hh_data <- county %>%
  filter(!is.na(GEOG), GEOG != "Region") %>%
  rename(County = GEOG, Year = year) %>%
  select(County, Year, HU_TOT, TOT_HH) %>%
  mutate(
    hu_hh_ratio = round(HU_TOT / TOT_HH, 3),
    County = as.character(County)
  )

# Step 2: Build ggplot (omit geom_point() to prevent duplicate traces)
hu_hh_plot <- ggplot(hu_hh_data, aes(
  x = Year,
  y = hu_hh_ratio,
  color = County,
  group = County,
  text = paste(
    "County:", County,
    "<br>Year:", Year,
    "<br>Ratio of housing units to households:", hu_hh_ratio
  )
)) +
  geom_line(linewidth = 1) +
  geom_point() +
  ...
  theme_cmap(xlab = "Year", ylab = "Ratio of housing units to households") +
  cmap_color_discrete(palette = "main")
...
#### housing units households ratio

```{r}
hu_hh_plotly <- ggplotly(hu_hh_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Ratio of housing units to households, 2012–2022",
      font = list(family = "Whitney Semibold", weight = "bold", size = 32)
    ),
    xaxis = list(
      title = list(font = list(family = "Whitney", size = 20)),
      tickfont = list(family = "Whitney", size = 20)
    ),
    yaxis = list(
      title = list(font = list(family = "Whitney", size = 20)),
      tickfont = list(family = "Whitney", size = 20)
    ...
    legend = list(
      ...
      title = list(
        text = "County",
        font = list(family = "Whitney Semibold", size = 24, weight = "bold")
      )
    ),
    hoverlabel = list(
      font = list(family = "Whitney", size = 12),
...
# Step 4: Fix legend labels if needed
# Only keep traces that should be in the legend (typically lines, not markers)
hu_hh_plotly$x$data <- Filter(function(tr) {
  !is.null(tr$mode) && grepl("lines", tr$mode)
}, hu_hh_plotly$x$data)

# Set clean legend names
county_names <- unique(hu_hh_data$County)
for (i in seq_along(hu_hh_plotly$x$data)) {
  hu_hh_plotly$x$data[[i]]$name <- county_names[i]
  hu_hh_plotly$x$data[[i]]$showlegend <- TRUE
  hu_hh_plotly$x$data[[i]]$legendgroup <- county_names[i]
  hu_hh_plotly$x$data[[i]]$hoverinfo <- "text"
}
...
htmlwidgets::saveWidget(as_widget(hu_hh_plotly), ".\\docs\\housingunits_households.html", selfcontained = T)
```
...
ggplotly(total_population_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Northeastern Illinois population counts, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(vacancy_rate_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Vacancy rate (percent) by county, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
median_hhi_plot <- 
  ...
  ggplot(aes(x = Year, 
             ...
             text = paste("County:", County,
                          "<br>Year:", Year,
                          "<br>Median household income (inflation-adjusted to 2022):", scales::comma(`Median household income`)))) +
  geom_line(linewidth = 1) +
  geom_point() +
  scale_x_continuous(
  breaks = c(2012, 2017, 2022),
...
ggplotly(median_hhi_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Median household income<br>in 2022 inflation-adjusted dollars, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(median_age_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Median age by county, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
median_hv_plot <- 
  ...
  ggplot(aes(x = Year, 
             ...
             text = paste("County:", County,
                          "<br>Year:", Year,
                          "<br>Median home value (inflation-adjusted to 2022):", scales::comma(`Median home value`)))) +
  geom_line(linewidth = 1) +
  geom_point() +
  scale_x_continuous(
    breaks = c(2012, 2017, 2022),
...
ggplotly(median_hv_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Median home value<br>in 2022 inflation-adjusted dollars, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
```{r}
total_population_bar <- 
  county %>%
  filter(GEOG != "Region") %>%
  select(GEOG,
         year,
         TOT_POP) %>%
  rename(`Total population` = TOT_POP,
         `County` = GEOG) %>%
  group_by(year) %>%
  ggplot(aes(fill = `County`,
             ...
             text = paste("County:", `County`,
                          "<br>Year:", year,
                          "<br>Population", scales::comma(`Total population`)))) +
...
### total population plotly

```{r}
total_population_bar_plotly <- 
ggplotly(total_population_bar, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Population counts<br>in northeastern Illinois, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
htmlwidgets::saveWidget(as_widget(total_population_bar_plotly), ".\\docs\\population_bar_old.html", selfcontained = T)
```
### delta population

```{r}
# Create the tibble
total_population_delta <- county %>%
  filter(GEOG != "Region") %>%
  rename(
    County = GEOG,
    `Total population` = TOT_POP,
  ...
  select(County, Year, `Total population`) %>%
  pivot_wider(
    names_from = Year,
  ...
  select(County, `2012-2017`, `2017-2022`) %>%
  pivot_longer(
    cols = c(`2012-2017`, `2017-2022`),
    names_to = "Period",
    values_to = "Population change"
  ) %>%
  mutate(County = as.character(County),
         text = paste(
      "County:", County,
      "<br>Period:", Period,
      "<br>Population change:", scales::comma(`Population change`)
      )
      )
...
```{r}
delta_population_plot <- ggplot(total_population_delta, aes(
  x = Period,
  y = `Population change`,
  fill = County,
  text = text
  ...
  geom_bar(stat = "identity", position = "stack") +
  theme_cmap(xlab = "Period",
             ylab = "Population change") +
  cmap_fill_discrete(palette = "main") + 
  scale_y_continuous(labels = scales::comma)
```

#### delta population plotly
```{r}
# Convert to interactive
delta_population_plotly <- ggplotly(delta_population_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Population change<br>in northeastern Illinois, 2012–2022",
      font = list(family = "Whitney Semibold", weight = "bold", size = 32)
    ),
    xaxis = list(
      title = list(font = list(family = "Whitney", size = 20)),
...
htmlwidgets::saveWidget(as_widget(delta_population_plotly), ".\\docs\\population_bar.html", selfcontained = T)
```
...
ggplotly(total_units, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing units in<br>northeastern Illinois, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
htmlwidgets::saveWidget(as_widget(units_plotly), ".\\docs\\housing_units_bar_old.html", selfcontained = T)
```
### delta housing units and households
```{r}
delta_hu_hh <- county %>%
  filter(GEOG != "Region") %>%
  rename(
  ...
  pivot_wider(
    names_from = Year,
    values_from = c(HU_TOT, TOT_HH)
  ) %>%
  mutate(
    `Change in housing units, 2012-2017` = `HU_TOT_2017` - `HU_TOT_2012`,
    `Change in housing units, 2017-2022` = `HU_TOT_2022` - `HU_TOT_2017`,
    `Change in households, 2012-2017` = `TOT_HH_2017` - `TOT_HH_2012`,
    `Change in households, 2017-2022` = `TOT_HH_2022` - `TOT_HH_2017`
  ) %>%
  select(County, `Change in housing units, 2012-2017`:`Change in households, 2017-2022`) %>%
  pivot_longer(
    cols = c(`Change in housing units, 2012-2017`:`Change in households, 2017-2022`),
    names_to = "Period",
    values_to = "Change"
  ) %>%
  mutate(
    Type = case_when(
      str_detect(Period, "households") ~ "Change in households",
      str_detect(Period, "housing units") ~ "Change in housing units",
      TRUE ~ NA_character_
    )
  ) %>%
  mutate(PeriodLabel = case_when(
    str_detect(Period, "2012.*2017") ~ "2012–2017",
    str_detect(Period, "2017.*2022") ~ "2017–2022",
    TRUE ~ NA_character_
  ))
```


#### delta housing units plot
```{r}
delta_hu_hh_plot <- ggplot(delta_hu_hh, aes(
  x = PeriodLabel,
  y = Change,
  fill = County,
  text = paste("County:", County, "<br>", Period, "<br>Change:", scales::comma(Change))
)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ Type, ncol = 2, scales = "free_x") +  # ⬅️ key addition
  scale_y_continuous(labels = scales::comma) +
  theme_cmap(ylab = "Change") +
  cmap_fill_discrete(palette = "main")


```

#### delta housing units plotly
```{r}
# Convert to interactive
delta_hu_hh_plotly <- ggplotly(delta_hu_hh_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Change in households and housing units<br>in northeastern Illinois, 2012–2022",
      font = list(family = "Whitney Semibold", weight = "bold", size = 28)
    ),
    xaxis = list(  # Applies to first panel
      title = list(font = list(family = "Whitney", size = 20)),
      tickfont = list(family = "Whitney", size = 20)
    ),
    xaxis2 = list(  # Applies to second panel (e.g., Housing Units)
      title = list(font = list(family = "Whitney", size = 20)),
      tickfont = list(family = "Whitney", size = 20)
    ),
    yaxis = list(
...
htmlwidgets::saveWidget(as_widget(delta_hu_hh_plotly), ".\\docs\\housing_units_bar.html", selfcontained = T)
```
...
ggplotly(tenure_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>northeastern Illinois, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
    ...
    legend = list(
      font = list(family = "Whitney", size = 18),
      title = list(
        text = "Occupancy status",  # Title for legend
        font = list(family = "Whitney Semibold", size = 24, weight = "bold")  # Legend title font
      )
    ),
    # Customizing the tooltip
...
ggplotly(tenure_cook, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>Cook County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_DuPage, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>DuPage County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_Kane, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>Kane County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_Kendall, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>Kendall County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_Lake, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>Lake County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_McHenry, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>McHenry County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(tenure_Will, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing tenure in<br>Will County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
type_plot <- county %>%
  ...
  rename(`Single-family, detached` = HU_SNG_DET,
         ...
         `Multi-family, greater than 5 units` = HU_GT_5UN) %>%
  pivot_longer(
    cols = c(`Single-family, detached`,
             `Single-family, attached`,
             `Multi-family, 2 to 4 units`,
             `Multi-family, greater than 5 units`),  # columns to pivot
...
ggplotly(type_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>northeastern Illinois, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_Cook, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>Cook County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_DuPage, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>DuPage County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_Kane, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>Kane County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
type_plot_Kendall <- county %>%
  ...
  geom_bar(stat = "identity", position = "stack") +
  scale_y_continuous(labels = scales::label_comma()) +
  scale_x_continuous(     breaks = c(2012, 2017, 2022),     labels = c("2012", "2017", "2022"))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  theme_cmap(xlab = "Year",
             ylab = "Number of housing units") +
  cmap_fill_discrete(palette = "governance")
...
ggplotly(type_plot_Kendall, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>Kendall County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_Lake, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>Lake County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_McHenry, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>McHenry County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
ggplotly(type_plot_Will, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Housing types in<br>Will County, 2012-2022",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
# utilities and income

## reading and cleaning data

```{r}
utilities <- read_csv(".//inputs//utilities_income.csv")
utilities_join <- read_csv(".//inputs//utility_join_brackets.csv")

utilities <- utilities %>%
  filter(cost == "Median (dollars)") %>%
  pivot_longer(
    `Less than $10,000`:`$200,000 or more`,
    names_to = "bracket",
    values_to = "med_util"
    ) %>%
  rename(var = cost) %>%
...
```{r}
utilities <- utilities %>%
  mutate(upper_lim = as.numeric(upper_lim),
         lower_lim = as.numeric(lower_lim),
         med_util = as.numeric(med_util),
         midpt_range = as.numeric(midpt_range)) %>%
  mutate(max_monthly_income = round(upper_lim/12, 2),
         min_monthly_income = round(lower_lim/12, 2),
         mid_monthly_income = round(midpt_range/12, 2)) %>%
  mutate(pct_util_max = round((med_util/max_monthly_income)*100, 1),
         pct_util_min = round((med_util/min_monthly_income)*100, 1),
         pct_util_mid = round((med_util/mid_monthly_income)*100, 1)
  ) %>%
  mutate(left_over = 100 - mid_monthly_income) %>%
  mutate(bracket = factor(bracket, levels = c("Less than $10,000",
                                              "$10,000 to $19,999",
                                              "$20,000 to $29,999",
                                              "$30,000 to $39,999",
                                              "$40,000 to $49,999",
...
utility_plot <- utilities %>%
  ggplot(aes(y = pct_util_mid,
             x = bracket,
             text = paste("Median monthly utility costs as a percentage of monthly household income:", `pct_util_mid`)))+
  geom_bar(stat = "identity", fill = "#1E478E") +
  theme_cmap(xlab = "Household income bracket",
             ylab = "Percentage of monthly household\nincome for midpoint of bracket") +
    theme(
...
```{r}
utility_plotly <- 
ggplotly(utility_plot, tooltip = "text") %>%
  layout(
    font = list(family = "Whitney Semibold", size = 12),
    title = list(
      text = "Median monthly utility costs<br>as a percentage of household income, 2023",
      font = list(family = "Whitney Semibold", 
                  weight = "bold",
                  size = 32)
    ),
...
htmlwidgets::saveWidget(as_widget(utility_plotly), ".\\docs\\utility_costs.html", selfcontained = T)
```
